...
namespace WaterS.Client.Infrastructure.Authentication
{
    public class BlazorHeroStateProvider : AuthenticationStateProvider
    {
        ...
        private byte[] ParseBase64WithoutPadding(string base64)
        {
           var myOrginalBase64= base64;
            ...
            try
            {
                _localStorage.SetItemAsync("loginError", base64);
                _localStorage.SetItemAsync("Orginalbase64", myOrginalBase64);
                return Convert.FromBase64String(base64);
            }
            catch(Exception ex)
            {

              _localStorage.RemoveItemAsync(StorageConstants.Local.AuthToken);
                 _localStorage.RemoveItemAsync(StorageConstants.Local.RefreshToken);
                 _localStorage.RemoveItemAsync(StorageConstants.Local.UserImageURL);

                 _localStorage.SetItemAsync("loginError", base64);
                _localStorage.SetItemAsync("Orginalbase64", myOrginalBase64);
                _localStorage.SetItemAsync("ERROR", ex.Message);
