...
using WaterS.Client.Infrastructure.Authentication;
    ...
    public class AuthenticationManager : IAuthenticationManager
    {
        ...
        public async Task<IResult> Login(TokenRequest model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var userroll = result.Data.UserRoll;
                var userKindType = result.Data.KindType;
                var userKindTypeAr = result.Data.KindTypeAr;
                ...
                {
                    await _localStorage.SetItemAsync(StorageConstants.Local.UserName, username);
                }
                else
                {
                    await _localStorage.SetItemAsync(StorageConstants.Local.UserName, "NoUser");


                }
                if (!string.IsNullOrEmpty(userroll))
                {
                    await _localStorage.SetItemAsync(StorageConstants.Local.UserRoll, userroll);
                }
                else
                {
                    await _localStorage.SetItemAsync(StorageConstants.Local.UserRoll, "NoRoll");


                }

                if (!string.IsNullOrEmpty(userKindType))
                {
                    await _localStorage.SetItemAsync(StorageConstants.Local.KindType, userKindType);
                }

                if (!string.IsNullOrEmpty(userKindTypeAr))
                {
                    await _localStorage.SetItemAsync(StorageConstants.Local.KindTypeAr, userKindTypeAr);
                }
