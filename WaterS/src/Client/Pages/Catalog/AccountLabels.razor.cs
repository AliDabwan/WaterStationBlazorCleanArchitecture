...
{
    public partial class AccountLabels
    {
        [Inject] private IAccountNameManager BottleTypeManager { get; set; }

        [CascadingParameter] private HubConnection HubConnection { get; set; }

        private List<GetAllAccountNamesResponse> _brandList = new();
        private GetAllAccountNamesResponse _brand = new();
        ...
        private async Task GetBottleTypessAsync()
        {
            ...
            if (response.Succeeded)
            {
                ...
                if (CurrentUserRool == RoleConstants.ManagerRole)
                {
                    ...
                    _brandList = response.Data.Where(x => x.CompanyId == mycomp) .ToList();

                }
                else if (CurrentUserRool == RoleConstants.StationRole)
                {

                    _brandList = response.Data.Where(x => x.CompanyId == mycomp && x.StationId == mystion ).ToList();

                }
                else if (CurrentUserRool == RoleConstants.DriverRole)
                {

                    _brandList = response.Data.Where(x => x.CompanyId == mycomp && x.StationId == mystion && x.DriverId == mydriver).ToList();
        ...
        {
            _brand = new GetAllAccountNamesResponse();
            await GetBottleTypessAsync();
        }

        private bool Search(GetAllAccountNamesResponse brand)
        {
            ...
            if (brand.CategoryType?.Contains(_searchString, StringComparison.OrdinalIgnoreCase) == true)
            {
                return true;
            }
            return false;
