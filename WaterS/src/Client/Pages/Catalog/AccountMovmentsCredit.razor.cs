...
{
    public partial class AccountMovmentsCredit
    {
        ...
        private async Task GetBottleTypessAsync()
        {
            ...
            //var response = await AccountMovmentManager.GetAllAsync();
            if (response.Succeeded)
            {
                _brandList = response.Data;

                _brandList = response.Data.Where(x => x.CreditAmmount > 0).ToList();
        ...
        {
            //var response = await AccountMovmentManager.ExportToExcelAsync(_searchString);
