...
{
    public partial class DriverRegions
    {
        ...
        private async Task LoadData(int pageNumber, int pageSize, TableState state)
        {
            ...
            var request = new GetAllPagedDriverRegionsRequest { PageSize = pageSize, PageNumber = pageNumber + 1,CompanyId=mycomp,StationId=mystion, SearchString = _searchString, Orderby = null };
            var response = await DriverRegionManager.GetDriverRegionsAsync(request);
            if (response.Succeeded)
            {
                _currentPage = response.CurrentPage;

                //if (CurrentUserRool=="Manager")
                //{
                //    _pagedData = response.Data.Where(x=>x.CompanyId==mycomp);
                //    _totalItems = _pagedData.Count();

                //}
                //else if (CurrentUserRool == "Station")
                //{
                //    _pagedData = response.Data.Where(x => x.StationId == mystion);
                //    _totalItems = _pagedData.Count();
                ...
                var requestCustomer = new GetAllPagedCustomersRequest { PageSize = 0, PageNumber = 0 + 1,CompanyId = mycomp, StationId = mystion, withOutInclud = 1, SearchString ="", Orderby = orderings };
                ...
                foreach (var subitem in _pagedData)
                {
                    ...
                    //_snackBar.Add(" subitem.CustomerList " + subitem.CustomerList.Count(), Severity.Success);

                    //foreach (var itemaddress in myCustomrtsList)
                    //{
                    //    subitem.CustomerList.Add(new Customer
                    //    {
                    ...
                    //people.Add(new GetAllPagedDriverRegionsResponse
                    //{
                    //    Id = subitem.Id,
                    //    RegionId = subitem.RegionId,
                    //    DriverId = subitem.DriverId,
                    //    CompanyId = subitem.CompanyId,
                    //    StationId = subitem.StationId,
                    //    Station = subitem.Station,
                    //    Company = subitem.Company,
                    //    Driver = subitem.Driver,
