...
{
    public partial class DriversIncluds
    {
            ...
            //{
                ...
                foreach (var subitem in _pagedData)
                {
                    people.Add(new GetAllPagedDriversResponse
                    {
                        ...
                        DriverRegionList = subitem.DriverRegionList,
                        //RegionsCount = subitem.RegionsCount,
                        Phone = subitem.Name,
                        Adress = subitem.Adress,
                        myStation = subitem.myStation
                    });
                regionSerial = 0;
                //foreach (var item in subitem.DriverRegionList)
                //{
                //    regionSerial++;
                //    item.serialId = regionSerial;
                //}
            }
        ...
        IList<GetAllPagedDriversResponse> people = new List<GetAllPagedDriversResponse>();
        IList<GetAllPagedDriverRegionsResponse> myRegionList = new List<GetAllPagedDriverRegionsResponse>();


        private async Task ShowBtnPressCustoms( int nr)
        {
            var tmpPerson = myRegionList.First(f => f.Id == nr);
            tmpPerson.ShowDetails = !tmpPerson.ShowDetails;
            ResetExampleCustom();
            OpenOverlayCustom();

        }
        private async  Task ShowBtnPressRegion(int nr, int drivId)
        {
            ...
            var request = new GetAllPagedDriverRegionsRequest { PageSize = 0, PageNumber = 0 + 1, CompanyId = mycomp,DriverId= drivId, StationId = mystion, SearchString = "", Orderby = null };
          ...
          int  serial = 0;
            ...
            if (myDrivers.Succeeded)
            {
                ...
                myRegionList = myDrivers.Data;
                //var myCustomrtsList = mycuslist.Where(x => x.DriverId == drivId && x.RegionId == regId);

                //serial = 0;
                OpenOverlay();
                dataLoadedCustomers = false;
                foreach (var item in myRegionList)
                {
                    //mycuslist.Clear();
                       serial++;
                    item.serial = serial;

                    var requestCustomer = new GetAllPagedCustomersRequest { PageSize = 0, PageNumber = 0 + 1, CompanyId = mycomp, StationId = item.StationId, DriverId = item.DriverId,RegionId= item.RegionId, withOutInclud = 1, SearchString = "", Orderby = null };
                    ...
                    if (myCustomrts.Succeeded)
                    {
                        ...
                        item.CustomerList = mycuslist;

                        item.CustomerCounts = mycuslist.Count;
                    }
                }
                OpenOverlayCustomerCount();
            }
            else
            {
        ...
        private bool dataLoadedCustomers;

        private bool isVisibleCustom;
        private bool dataLoadedCustom;
        ...
        public async void OpenOverlayCustomerCount()
        {
            await Task.Delay(1000);

            dataLoadedCustomers = true;


            StateHasChanged();
        }

        public async void OpenOverlayCustom()
        {
            isVisibleCustom = true;
            await Task.Delay(3000);
            isVisibleCustom = false;
            dataLoadedCustom = true;
            StateHasChanged();
        }
        ...
        public void ResetExampleCustom()
        {
            dataLoadedCustom = false;
        }
